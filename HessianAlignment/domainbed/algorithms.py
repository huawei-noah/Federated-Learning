...
class HGP(Algorithm):
    ...
    def update(self, minibatches, unlabeled=False):
        ...
        grads_of_norm_of_grad = [autograd.grad(env['sadg'], self.classifier.parameters(), create_graph=True,retain_graph=True) for env in envs]
...
class Hutchinson(Algorithm):
    ...
    def update(self, minibatches, unlabeled=False):
        ...
        if len(envs) > 0:
            for i in range(len(all_flatten_grads)):
                sadg_penalty_list.append(self.penalty_alpha *(all_hessians[i] - mean_hessian.detach()).pow(2).sum() + self.penalty_beta * (all_flatten_grads[i] - flatten_mean_grad.detach()).pow(2).sum() )

            N = len(sadg_penalty_list)
            sadg_penalty = torch.stack(sadg_penalty_list).sum()/len(envs)
        else:
             sadg_penalty = torch.stack([self.penalty_alpha * torch.flatten(all_hessians[0]).pow(2).sum(),self.penalty_beta* envs[0]['sadg']]).sum()
