...
def _hparams(algorithm, dataset, random_seed):
    ...
    # elif algorithm == "Hutchinson":
    #     _hparam('penalty_alpha', 0.001,  lambda r: 10**r.uniform(-1, -3))
    #     _hparam('penalty_beta', 0.005, lambda r: 10**r.uniform(-2, -4))

    # elif algorithm == "Hutchinson":
    #     _hparam('penalty_alpha', 0.001,  lambda r: 10**r.uniform(1, -2))
    #     _hparam('penalty_beta', 0.005, lambda r: 10**r.uniform(1, -2))

    # Dataset-and-algorithm-specific hparam definitions. Each block of code
    # below corresponds to exactly one hparam. Avoid nested conditionals.

    if dataset in SMALL_IMAGES:
        if algorithm == "SADG_simple_implementation_var" or algorithm == "SADG_simple_implementation_Hutch_var":
            _hparam('lr', 1e-3, lambda r: 10**r.uniform(-4.5, -3.5))
        else:
    ...
    if dataset in SMALL_IMAGES:
        _hparam('batch_size', 64, lambda r: int(2**r.uniform(4, 9)))
    elif algorithm == 'ARM':
        _hparam('batch_size', 8, lambda r: 8)
    elif dataset == 'DomainNet':
        _hparam('batch_size', 32, lambda r: int(2**r.uniform(4, 5)))
    else:
